// @ts-nocheck
// Standalone seed file — no path aliases, runs directly via `bun prisma/seed-standalone.ts`
// Use this if `bun prisma/seed.ts` fails with "Cannot find module '@/lib/db'"
...
/** Compute and store the minPrice for an event based on its ticket tiers. */
async function updateEventMinPrice(eventId: string): Promise<void> {
  const tiers = await db.ticketTier.findMany({
    where: { eventId },
    select: { price: true },
  });
  if (tiers.length === 0) return;
  const prices = tiers.map((t: any) => parseFloat(t.price.toString()));
...
async function main() {
  ...
  const categories = await Promise.all([
    db.category.upsert({ where: { slug: "music" }, update: {}, create: { nameAr: "موسيقى", nameEn: "Music", slug: "music", description: "حفلات موسيقية ومهرجانات" } }),
    db.category.upsert({ where: { slug: "sports" }, update: {}, create: { nameAr: "رياضة", nameEn: "Sports", slug: "sports", description: "فعاليات رياضية ومباريات" } }),
    db.category.upsert({ where: { slug: "culture" }, update: {}, create: { nameAr: "ثقافة", nameEn: "Culture", slug: "culture", description: "فعاليات ثقافية ومعارض فنية" } }),
    db.category.upsert({ where: { slug: "tech" }, update: {}, create: { nameAr: "تقنية", nameEn: "Technology", slug: "tech", description: "مؤتمرات تقنية وورش عمل" } }),
    db.category.upsert({ where: { slug: "food" }, update: {}, create: { nameAr: "طعام", nameEn: "Food", slug: "food", description: "مهرجانات طعام وتجارب طهي" } }),
    db.category.upsert({ where: { slug: "business" }, update: {}, create: { nameAr: "أعمال", nameEn: "Business", slug: "business", description: "مؤتمرات أعمال وندوات" } }),
    db.category.upsert({ where: { slug: "entertainment" }, update: {}, create: { nameAr: "ترفيه", nameEn: "Entertainment", slug: "entertainment", description: "فعاليات ترفيهية وعروض" } }),
    db.category.upsert({ where: { slug: "education" }, update: {}, create: { nameAr: "تعليم", nameEn: "Education", slug: "education", description: "ورش عمل ودورات تعليمية" } }),
  ]);
